        ...
        <div class="flex flex-col gap-3 p-3 w-full border-[1px] items-center">
            <div class="flex gap-5 w-full">
                ...
                <div  class="flex flex-col w-full">
                    ...
                    <label>Product :</label>
                    <select placeholder="fhewjr" class="border rounded p-2" v-model="selectedProduct">
                        <option value="" disabled>Select an option</option>
                        <option v-for="product in productList" :key="product.id" :value="product">{{ product.name }}</option>
                    </select>
                </div>

                <div class="flex flex-col w-60 ">
                    <label>Price :</label>
                    <select class="border rounded p-2" v-model="selectedPrice">
            ...
            <div class="flex gap-4 w-full ">
                <div  class="flex flex-col w-fit items-center w-50 ">
                    <BaseInput placeholder="Qnty" type="number"  v-model="productQnty"/>
                </div>
                <div   class="flex flex-col w-full ">
                    <BaseInput placeholder="Total" class="w-full" readonly type="number"  v-model="productTotal"/>
                </div>
                <BaseButton class="w-full text-[22px]" @click="addProduct">+</BaseButton> 
            ...
            <table class="table w-full ">
                ...
                <tbody>
                    ...
                    <tr v-for="(item, index) in purchaseList" :key="index" class="flex gap-2 w-full border-1 bg-gray-100">
                        ...
                        <td class="w-20">{{ item.product }}</td>
                        <td class="w-15 bg-red-200">{{ item.price }}</td>
                        <td class="w-10 ">{{ item.qnty }}</td>
                        <td class="w-20 bg-yellow-100">{{ item.total }}</td>
                        <td><button @click="deletePurchaseItem(index)" class="bg-red-100 text-red-500 p-1 px-2">X</button></td>
                    </tr>
